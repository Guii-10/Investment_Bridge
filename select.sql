...
SELECT * FROM PROJETOS WHERE VALOR_TOTAL >= 700000.00;
...
SELECT FK_CLIENTE_ID AS Cliente, COUNT(FK_PROJETOS_COD) AS Quantidade_de_Projetos FROM CLIENTE_PROJETO GROUP BY FK_CLIENTE_ID;

#Quantos clientes o investidor tem
SELECT FK_INVESTIDOR_ID AS Investidor, COUNT(FK_CLIENTE_ID) AS Clientes FROM INVESTIDOR_CLIENTE GROUP BY FK_INVESTIDOR_ID;

#Quantos projetos o investidor tem
SELECT FK_INVESTIDOR_ID AS Investidor, COUNT(FK_PROJETOS_COD) AS Quantidade_De_Projetos FROM INVESTIDOR_PROJETO GROUP BY Investidor;